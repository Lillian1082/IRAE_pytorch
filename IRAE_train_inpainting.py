...
from datasets.celebaHQ import CelebA_HQ
# from datasets.moons import MOONS
import matplotlib.pyplot as plt
from Model.UNet import UNet
from Model.Partial_Conv import PConvUNet
...
# parser.add_argument('--generate', action='store_false', help='Generate samples from a model.')
...
def fetch_dataloader(args, train=True, data_dependent_init=False):
    args.input_dims = {'mnist': (3,32,32), 'celeba': (3,256,256), 'flower': (3,128,128), 'bird': (3,128,128), 'celeba_HQ': (3, 256, 256), 'moons': (32,32)}[args.dataset] # 'celeba': (3,64,64) for image denoising
    transforms = {'mnist': T.Compose([T.Pad(2),                                         # image to 32x32 same as CIFAR
                  ...
                  'celeba_HQ': T.Compose([T.Resize((256, 256)),  # for image inpainting, mask shape 32*32
                                       T.Lambda(lambda im: np.array(im, dtype=np.float32)),  # to numpy
                                       T.Lambda(lambda x: np.floor(x / 2 ** (8 - args.n_bits)) / 2 ** args.n_bits),
                                       # lower bits
                                       T.ToTensor(),
    ...
    dataset = {'mnist': MNIST, 'celeba': CelebA, 'celeba_HQ': CelebA_HQ, 'flower': Flower, 'bird': Bird}[args.dataset] #, 'moons': MOONS
    ...
    if train:
        dataset = dataset(root=args.data_dir, args=args, train=train, transform=transforms, mask_transform=mask_transform) # , mini_data_size=10
    else:
    ...
    batch_size = args.batch_size if train else 1 # args.batch_size_init if data_dependent_init else args.batch_size  # if data dependent init use init batch size
    kwargs = {'num_workers': 1, 'pin_memory': True} if args.device.type is 'cuda' else {}
    return DataLoader(dataset, batch_size=batch_size, shuffle=False, drop_last=True, sampler=sampler, **kwargs) # train: shuffle=(sampler is None)
...
def train_epoch(model, dataloader, optimizer, writer, epoch, last_epoch, args):
    ...
    for i, (clean, corrupted, mask) in enumerate(dataloader):
        ...
        if args.archi=='FFDNet':
            ...
            if args.noise_mode=='B':
                ...
                stdn = np.ones(noise.size()[0])*args.noise_level

            for nx in range(noise.size()[0]):
                sizen = noise[0, :, :, :].size()
                noise[nx, :, :, :] = torch.FloatTensor(sizen).normal_(mean=0, std=stdn[nx]/255)
            ...
            clean, corrupted, mask = clean.cuda(), corrupted.cuda(), mask.cuda()
            if args.archi == 'PartialConv':
                output, _ = model(corrupted, mask)
            else:
        ...
        with torch.no_grad():
            ...
            if args.learn_residual: # learn noise for denoise tasks
                ...
                if args.inpainting_mode == 'irregular':
                    output = output * (1. - mask) + corrupted * mask
                elif args.inpainting_mode == 'center':
                    output = output * mask + corrupted * (1. - mask)
...
def denoise(model, test_dataloader, writer, epoch, Best_PSNR, best_epoch, args):
    ...
    avg_percent_psnr_output = [0, 0, 0, 0]
    avg_percent_ssim_output = [0, 0, 0, 0]
    avg_percent_l1_output = [0, 0, 0, 0]
    avg_percent_l2_output = [0, 0, 0, 0]
    count = [0, 0, 0, 0]

    avg_time = 0
    for i, (clean, corrupted, mask) in enumerate(test_dataloader):
        t0 = time.time()
        ...
        if args.inpainting_mode == 'irregular':
            avg_percent_psnr_output[int(percent//10) -1] += psnr_output
            avg_percent_ssim_output[int(percent//10) -1] += score_ssim
            # avg_percent_l1_output[int(percent//10) -1] += L1_loss
            # avg_percent_l2_output[int(percent//10) -1] += L2_loss
            count[int(percent//10) -1] += 1
        ...
        if args.inpainting:
            save_image(image, os.path.join(args.output_dir, args.dataset+'inpainting_gt_image_%d.png'%i)) # 'gt_image_{}.png'.format(i)
            save_image(noisy_image, os.path.join(args.output_dir, args.dataset+'_inpainting_'+args.inpainting_mode+'_corrupted_image_%d.png'%i)) # 'corrupted_image_{}.png'.format(i)
            save_image(denoised_image, os.path.join(args.output_dir, args.dataset+'_inpainting_'+args.inpainting_mode+'_'+args.archi+'_reconstructed_image_%d.png'%i)) # 'denoised_image_{}.png'.format(i)
    ...
    if args.inpainting_mode == 'irregular':
        # print('Count:', count)
        for k, num in enumerate(count):
            if num:
                avg_percent_psnr_output[k] /= num
                avg_percent_ssim_output[k] /= num
        for k in range(4):
            print('%d'%(10*(k+1)) + '%-' +'%d'%(10*(k+2)) + '%:', 'count:', count[k], 'PSNR:', avg_percent_psnr_output[k], 'SSIM:', avg_percent_ssim_output[k], 'L1:', avg_percent_l1_output[k], 'L2:', avg_percent_l2_output[k])
...
if __name__ == '__main__':
    ...
    Dataset_List = ['celeba', 'flower', 'bird', 'celeba_HQ'] # Synthetic Datasets for Denoising; Impainting; JPEG decompression
    if args.dataset in Dataset_List:
        ...
        DDataset = Dataset_Grad(args.train_dataPath, randomCount=args.randomCount, augment=False, cropPatch=True, \
                                 cropSize=args.cropSize, real=1, denoise=args.denoise, decompression=args.decompression, \
        ...
        train_dataloader = DataLoader(dataset=DDataset, num_workers=4, drop_last=True, batch_size=args.batch_size, shuffle=True)
        VDataset = Dataset_Grad(args.test_dataPath, randomCount=1, augment=0, cropPatch=0, \
                                 cropSize=args.cropSize, real=1, denoise=args.denoise, decompression=args.decompression, \
                                decompression_level=args.decompression_level, noiseLevel=args.noise_level, radius=args.radius, \
                                noise_estimate=args.noise_estimate)
        test_dataloader = DataLoader(dataset=VDataset, num_workers=4, batch_size=150, shuffle=False)
    ...
    if args.inpainting:
        print('args.inpainting', args.inpainting)
